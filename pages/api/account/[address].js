    ...
    switch(method) {
        ...
        case 'PUT':
            try {
                ...
                const account = await Account.findOneAndUpdate({ address: queryAddress }, { ...queryParams })

                if (account) return res.status(200).json({ success: true, data: account })
        ...
        case 'DELETE':
            try {
                const deletedAccount = await Account.deleteOne({ address: queryAddress })
                
                if (deletedAccount) return res.status(200).json({ success: true, data: deletedAccount })
                else return res.status(404).json({ success: false, data: 'Account does not exist.' })
            } catch (error) {
                return res.status(400).json({ success: false, error: error.message })
            }
